...
const FAQS_EN = [
  ...
  { q: 'Is there a free plan?', a: 'No. Nina Purple has one membership, $20/month, with no free tier.\n\nEvery interaction on Nina Purple — a message, a profile unlock, a compatibility answer — runs on real infrastructure: servers, cooling systems, and electricity drawn from data centers. We don\'t think that cost should be invisible, and we don\'t think "free" is honest — someone always pays for the compute, whether that\'s us, an advertiser monetizing your attention, or the environment.\n\nCharging a flat, transparent membership fee lets us run the platform without selling your data or your attention to advertisers, cover the real infrastructure cost of every match, message, and unlock, and keep our per-interaction pricing tied to actual compatibility value, not manufactured scarcity.\n\nYour $20/month covers unlimited browsing, 21 compatibility questions, 1 free profile unlock, 30 free messages, full community room access, and Nina Purple Experiences. Additional unlocks and messages beyond those allowances are billed individually and scaled by match quality — the better the match, the lower the cost per message, so meaningful connection is always the cheapest thing on the platform.' },
...
const FAQS_FR = [
  ...
  { q: 'Y a-t-il un plan gratuit ?', a: "Non. Nina Purple offre une seule adhésion, 20$/mois, sans palier gratuit.\n\nChaque interaction sur Nina Purple — un message, un déverrouillage de profil, une réponse de compatibilité — fonctionne sur une infrastructure réelle : serveurs, systèmes de refroidissement et électricité tirée des centres de données. Nous ne pensons pas que ce coût devrait être invisible, et nous ne pensons pas que « gratuit » soit honnête — quelqu'un paie toujours pour le calcul, que ce soit nous, un annonceur qui monétise votre attention, ou l'environnement.\n\nFacturer un abonnement forfaitaire et transparent nous permet de faire fonctionner la plateforme sans vendre vos données ou votre attention aux annonceurs, de couvrir le coût réel d'infrastructure de chaque correspondance, message et déverrouillage, et de maintenir notre tarification par interaction liée à la valeur de compatibilité réelle, non à une rareté artificielle.\n\nVos 20$/mois couvrent la navigation illimitée, 21 questions de compatibilité, 1 déverrouillage de profil gratuit, 30 messages gratuits, l'accès complet aux salons communautaires et les Expériences Nina Purple. Les déverrouillages et messages supplémentaires au-delà de ces allocations sont facturés individuellement et ajustés selon la qualité de la correspondance — meilleure est la correspondance, plus bas est le coût par message, pour que la connexion significative soit toujours ce qu'il y a de moins cher sur la plateforme." },
